...
/* ECALL host services live in dbt_common.c (rv32_handle_ecall) so the
 * interpreter and JIT cannot drift apart on open flags, dirops, etc. */
    ...
    for (;;) {
        ...
        case OP_SYSTEM:
            if (insn.imm == 0 && insn.funct3 == 0) {
                /* ECALL */
                state->pc = next_pc;
                int rc = rv32_handle_ecall(state->x, bin, next_pc - 4);
                if (rc != -2) return rc;
                state->x[0] = 0;
                continue;
            ...
            case 0x1C: /* FMV.X.W / FCLASS */
                if (fmt == 0) {
                    if (insn.funct3 == 0) {
                        ...
                        uint32_t bits;
                        memcpy(&bits, &a, 4);
                        if (insn.rd) state->x[insn.rd] = rv32_fclass_s(bits);
                    }
                    ...
                    uint64_t bits;
                    memcpy(&bits, &a, 8);
                    if (insn.rd) state->x[insn.rd] = rv32_fclass_d(bits);
                }
                break;
            case 0x1E: /* FMV.W.X */
