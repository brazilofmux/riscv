/* dbt_x64.c — x86-64 code emission for the RV32IMFD DBT.
 *
 * Architecture-neutral plumbing (block cache, ECALL handler, run loop,
 * init/cleanup) lives in dbt_common.c. This file provides the three arch
 * hooks declared in dbt.h: dbt_translate_block, dbt_emit_trampoline,
 * dbt_jit_available — plus everything they pull in (per-instruction code
 * generation, register cache, intrinsic stubs).
 *
 * Host register convention (System V ABI):
 *   RBX = pointer to rv32_ctx_t
 *   R12 = guest memory base
 *   R13 = block cache base (for inline-cache chained exits)
 *   RAX/RCX/RDX = scratch
 *   RSI/RDI/R8-R11/R14/R15 = 8 LRU register cache slots
 */
#include "dbt.h"
#include "decoder.h"
#include "emit_x64.h"
...
/* Inline-cache probe sequences need this as a constexpr; the neutral
 * version inside dbt_common.c is static. */
